...
export type CompetitionNotificationPrefs = {
    enabled: boolean;
    matchStart: boolean;
    halftime: boolean;
    matchEnd: boolean;
...
type CompetitionNotificationModalProps = {
    visible: boolean;
    initialPrefs: CompetitionNotificationPrefs;
    onClose: () => void;
    onSave: (prefs: CompetitionNotificationPrefs) => void;
};
...
// Global styles object (will be populated per render)
let styles: ReturnType<typeof createStyles>;

export function CompetitionNotificationModal({
    visible,
    initialPrefs,
    onClose,
    onSave,
}: CompetitionNotificationModalProps) {
    const { colors } = useAppTheme();
    const { t } = useTranslation();
    styles = useMemo(() => createStyles(colors), [colors]); // Assign to global for CheckboxRow
    const [prefs, setPrefs] = useState<CompetitionNotificationPrefs>(initialPrefs);
    useEffect(() => {
        if (!visible) {
            return;
        }
    ...
    const updatePref = (key: keyof CompetitionNotificationPrefs, value: boolean) => {
        ...
        <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
            <Pressable style={styles.backdrop} onPress={onClose}>
                <Pressable style={styles.sheet} onPress={(e) => e.stopPropagation()}>
                    ...
                    <Text style={styles.title}>{t('notifications.competition.title', { defaultValue: 'Définir les alertes' })}</Text>
