...
describe('usePlayerMatches', () => {
  ...
  it('falls back to legacy strategy when aggregate matches have no usable player stats', async () => {
    appEnv.mobileEnableBffPlayerAggregates = true;
    ...
    await waitFor(() => {
      expect(result.current.matches).toHaveLength(1);
      expect(result.current.matches[0]?.playerStats.goals).toBe(1);
      expect(result.current.matches[0]?.playerStats.rating).toBe('7.8');
    });

    queryClient.clear();
  });

  it('falls back to legacy strategy when aggregate only provides minutes without rating or event stats', async () => {
    appEnv.mobileEnableBffPlayerAggregates = true;
  ...
  it('falls back to legacy strategy when aggregate returns an empty list', async () => {
    appEnv.mobileEnableBffPlayerAggregates = true;
