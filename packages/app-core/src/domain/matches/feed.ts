...
import type {
  ComposeMatchesFeedInput,
  ComposeMatchesFeedResult,
...
export const filterSectionsByStatus = (
...
export const sortMatchesByKickoff = (matches: MatchFeedMatch[]): MatchFeedMatch[] => {
  return [...matches].sort(
...
export const buildFollowsSection = (
  sections: MatchCompetitionSection[],
...
export const composeMatchesFeed = (
  input: ComposeMatchesFeedInput,
): ComposeMatchesFeedResult => {
  const competitionCatalogMap = buildCompetitionCatalogMap(input.catalog);
  const normalizedSections = applyCompetitionCatalogToSections(
    input.baseSections,
  ...
  const followsSection = buildFollowsSection(
    filteredSections,
    input.followedTeamIds,
    input.followedMatchIds,
    input.followsSectionLabel,
  );
  const visibleCompetitionSections = filteredSections.filter(
    section => !input.hiddenCompetitionIds.includes(section.id),
  );
