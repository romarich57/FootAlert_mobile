...
test('GET /v1/teams/:id/full and /v1/players/:id/full return additive aggregates', async t => {
  ...
  assert.equal(teamPayload._hydration.status, 'core_ready');
  assert.equal(teamPayload._hydration.sections.advancedStats.state, 'loading');
  assert.equal(teamPayload._hydration.sections.statsPlayers.state, 'loading');
  assert.equal(teamPayload._hydration.sections.transfers.state, 'loading');
  assert.equal(teamPayload.response.advancedStats.response, null);
  assert.deepEqual(teamPayload.response.statsPlayers.response, []);
  assert.deepEqual(teamPayload.response.transfers.response, []);
  assert.equal(playerPayload.response.details.response[0].player.name, 'Kylian Mbappe');
  assert.equal(playerPayload._hydration.status, 'core_ready');
  assert.equal(playerPayload._hydration.sections.statsCatalog.state, 'loading');
  assert.equal(playerPayload._hydration.sections.career.state, 'loading');
  assert.equal(playerPayload._hydration.sections.matches.state, 'loading');
  assert.equal(playerPayload.response.statsCatalog.response, null);
