...
function readCacheBackend(rawValue, fallback) {
    const normalized = rawValue?.trim().toLowerCase();
    if (normalized === 'redis') {
        return 'redis';
    }
...
function readNotificationsPersistenceBackend(rawValue, fallback) {
    const normalized = rawValue?.trim().toLowerCase();
...
function readAttestationEnforcementMode(rawValue) {
    const normalized = rawValue?.trim().toLowerCase();
...
function readHostList(rawValue) {
...
function readNodeRole(rawValue) {
    const normalized = rawValue?.trim().toLowerCase();
...
function defaultCacheBackendForEnv(appEnv) {
    if (appEnv === 'production' || appEnv === 'staging') {
        return 'redis';
    }
    return 'memory';
}
function defaultNotificationsPersistenceBackendForEnv(appEnv) {
    if (appEnv === 'production' || appEnv === 'staging') {
        return 'postgres';
    }
    return 'memory';
...
export const env = {
    ...
    apiFootballBaseUrl: normalizeUrl(process.env.API_FOOTBALL_BASE_URL?.trim() || DEFAULT_API_BASE_URL),
    ...
    cacheTtlJitterPct: readNonNegativeInt(process.env.CACHE_TTL_JITTER_PCT, DEFAULT_CACHE_TTL_JITTER_PCT),
    cacheLockTtlMs: readPositiveInt(process.env.CACHE_LOCK_TTL_MS, DEFAULT_CACHE_LOCK_TTL_MS),
    cacheCoalesceWaitMs: readPositiveInt(process.env.CACHE_COALESCE_WAIT_MS, DEFAULT_CACHE_COALESCE_WAIT_MS),
    upstreamGlobalRpmLimit: readNonNegativeInt(process.env.UPSTREAM_GLOBAL_RPM_LIMIT, DEFAULT_UPSTREAM_GLOBAL_RPM_LIMIT),
    upstreamCircuitBreakerWindowMs: readPositiveInt(process.env.UPSTREAM_CIRCUIT_BREAKER_WINDOW_MS, DEFAULT_UPSTREAM_CIRCUIT_BREAKER_WINDOW_MS),
    ...
    mobileSessionTokenTtlMs: readPositiveInt(process.env.MOBILE_SESSION_TOKEN_TTL_MS, DEFAULT_MOBILE_SESSION_TOKEN_TTL_MS),
    mobileRefreshTokenTtlMs: readPositiveInt(process.env.MOBILE_REFRESH_TOKEN_TTL_MS, DEFAULT_MOBILE_REFRESH_TOKEN_TTL_MS),
    mobileAuthChallengeTtlMs: readPositiveInt(process.env.MOBILE_AUTH_CHALLENGE_TTL_MS, DEFAULT_MOBILE_AUTH_CHALLENGE_TTL_MS),
    mobileAttestationAcceptMock: readBoolean(process.env.MOBILE_ATTESTATION_ACCEPT_MOCK, DEFAULT_MOBILE_ATTESTATION_ACCEPT_MOCK),
    mobileAttestationEnforcementMode: readAttestationEnforcementMode(process.env.MOBILE_ATTESTATION_ENFORCEMENT_MODE),
    ...
    mobilePlayIntegrityServiceAccountEmail: readOptionalValue(process.env.MOBILE_PLAY_INTEGRITY_SERVICE_ACCOUNT_EMAIL),
    mobilePlayIntegrityServiceAccountPrivateKey: readOptionalValue(process.env.MOBILE_PLAY_INTEGRITY_SERVICE_ACCOUNT_PRIVATE_KEY),
    ...
    mobileAppAttestVerificationSecret: readOptionalValue(process.env.MOBILE_APP_ATTEST_VERIFICATION_SECRET),
    paginationCursorSecret: '',
    paginationCursorTtlMs: readPositiveInt(process.env.PAGINATION_CURSOR_TTL_MS, DEFAULT_PAGINATION_CURSOR_TTL_MS),
    notificationsBackendEnabled: readBoolean(process.env.NOTIFICATIONS_BACKEND_ENABLED, DEFAULT_NOTIFICATIONS_BACKEND_ENABLED),
    notificationsEventIngestEnabled: readBoolean(process.env.NOTIFICATIONS_EVENT_INGEST_ENABLED, DEFAULT_NOTIFICATIONS_EVENT_INGEST_ENABLED),
    notificationsPersistenceBackend: readNotificationsPersistenceBackend(process.env.NOTIFICATIONS_PERSISTENCE_BACKEND, defaultNotificationsPersistenceBackend),
    notificationsFanoutMaxPerEvent: readPositiveInt(process.env.NOTIFICATIONS_FANOUT_MAX_PER_EVENT, DEFAULT_NOTIFICATIONS_FANOUT_MAX_PER_EVENT),
    notificationsDeferredPromotionBatch: readPositiveInt(process.env.NOTIFICATIONS_DEFERRED_PROMOTION_BATCH, DEFAULT_NOTIFICATIONS_DEFERRED_PROMOTION_BATCH),
    notificationsDeferredDelayMs: readPositiveInt(process.env.NOTIFICATIONS_DEFERRED_DELAY_MS, DEFAULT_NOTIFICATIONS_DEFERRED_DELAY_MS),
    bffEnablePlayerCanonicalCacheKeys: readBoolean(process.env.BFF_ENABLE_PLAYER_CANONICAL_CACHE_KEYS, DEFAULT_BFF_ENABLE_PLAYER_CANONICAL_CACHE_KEYS),
    bffEnablePlayerMatchesSwr: readBoolean(process.env.BFF_ENABLE_PLAYER_MATCHES_SWR, DEFAULT_BFF_ENABLE_PLAYER_MATCHES_SWR),
    bffEnablePlayerOverviewRoute: readBoolean(process.env.BFF_ENABLE_PLAYER_OVERVIEW_ROUTE, DEFAULT_BFF_ENABLE_PLAYER_OVERVIEW_ROUTE),
...
for (const key of LEGACY_HMAC_ENV_KEYS) {
    if (readOptionalValue(process.env[key])) {
        throw new Error(`Deprecated legacy HMAC configuration "${key}" is forbidden. Use mobile attestation + session tokens.`);
    }
}
