...
function buildCircuitRedisKey(family) {
 ...
 */
async function syncCircuitStateFromRedis(family) {
 ...
 */
function publishCircuitStateToRedis(family, state) {
    if (!quotaRedisClient || !quotaRedisReady)
        return;
...
function isCircuitOpenLocal(family, now = Date.now()) {
    return now < getCircuitState(family).openUntilMs;
}
async function isCircuitOpen(family, now = Date.now()) {
    ...
    for (let attempt = 0; attempt <= env.apiMaxRetries; attempt += 1) {
        ...
        try {
            ...
            if (hasApiError) {
                ...
                throw new UpstreamBffError(isRateLimit ? 429 : 403, isRateLimit ? 'UPSTREAM_API_RATE_LIMIT' : 'UPSTREAM_API_ERROR', isRateLimit ? 'API-Football rate limit reached' : 'API-Football business error', rawErrors);
            }
