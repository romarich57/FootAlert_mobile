...
const seenNonceStore = new Map();
function computeMobileRequestSignature(payload, key) {
...
function isSameSignature(providedSignature, expectedSignature) {
...
function isReplayNonce(nonce, now, maxSkewMs) {
...
function resolvePathWithQuery(request) {
    return request.raw.url || request.url;
}
export function verifyMobileRequestAuth(request, options) {
...
export function resetMobileRequestNonceStoreForTests() {
