...
import { getReadStore, } from './readStore/runtime.js';
...
export async function buildReadinessPayload() {
    ...
    const status = redisCheck.ready && postgresCheck.ready && queueCheck.ready && upstreamGuardCheck.ready
        ? 'ready'
        : 'degraded';
    return {
        status,
...
export async function renderPrometheusMetrics() {
    ...
    if (readStoreBacklog) {
        appendMetricBlock(lines, 'footalert_bff_readstore_refresh_queued', 'gauge', 'Number of snapshot refresh jobs queued in read-store.', [`footalert_bff_readstore_refresh_queued ${readStoreBacklog.queued}`]);
        appendMetricBlock(lines, 'footalert_bff_readstore_refresh_in_progress', 'gauge', 'Number of snapshot refresh jobs in progress in read-store.', [`footalert_bff_readstore_refresh_in_progress ${readStoreBacklog.inProgress}`]);
        appendMetricBlock(lines, 'footalert_bff_readstore_refresh_failed', 'gauge', 'Number of failed snapshot refresh jobs in read-store.', [`footalert_bff_readstore_refresh_failed ${readStoreBacklog.failed}`]);
    }
