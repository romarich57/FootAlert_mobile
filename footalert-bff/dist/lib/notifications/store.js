...
function toEventStatusOnInsert() {
    return 'received';
}
...
function encodeDeliveriesCursor(value) {
    return Buffer.from(JSON.stringify(value), 'utf8').toString('base64url');
}
function decodeDeliveriesCursor(cursor) {
    if (!cursor) {
        return null;
    }
    try {
...
class InMemoryNotificationsStore {
    devices = new Map();
    ...
    async revokeDeviceByTokenHash(input) {
    ...
    async deleteByAuthSubject(input) {
        const removedDeviceIds = new Set([...this.devices.values()]
    ...
    async upsertSubscriptions(input) {
    ...
    async listSubscriptions(input) {
        ...
        return [...this.subscriptions.values()].filter(subscription => subscription.deviceId === activeDevice.id
            && subscription.scopeKind === input.scopeKind
            && subscription.scopeId === input.scopeId);
    }
    async insertEventIfAbsent(eventPayload) {
    ...
    async findEligibleDevicesForEvent(event) {
        const activeDevices = new Map([...this.devices.values()]
        ...
        for (const subscription of this.subscriptions.values()) {
            ...
            const matchesScope = (subscription.scopeKind === 'match' && Boolean(event.fixtureId) && subscription.scopeId === event.fixtureId)
    ...
    async listPendingDeliveriesPage(input) {
        ...
        const deliveries = [...this.deliveries.values()]
            .filter(delivery => delivery.eventId === input.eventId
            && delivery.status === 'pending')
            .sort((a, b) => compareCursor({ createdAt: a.createdAt, deliveryId: a.id }, { createdAt: b.createdAt, deliveryId: b.id }));
        const filteredDeliveries = cursor
            ? deliveries.filter(delivery => compareCursor({ createdAt: delivery.createdAt, deliveryId: delivery.id }, cursor) > 0)
            : deliveries;
    ...
    async countDeferredDeliveries(input) {
        return [...this.deliveries.values()].filter(delivery => delivery.eventId === input.eventId && delivery.status === 'deferred').length;
    }
    ...
    async markDeliveryOpenedByEventAndDevice(input) {
        const matchingDeviceIds = new Set([...this.devices.values()]
            .filter(device => device.authSubject === input.authSubject && device.deviceId === input.deviceId)
            .map(device => device.id));
...
class PostgresNotificationsStore {
    client;
    constructor(client) {
        this.client = client;
    }
    ...
    async listPendingDeliveriesPage(input) {
        const cursor = decodeDeliveriesCursor(input.cursor);
        const limit = Math.max(1, Math.floor(input.limit));
        const params = [input.eventId, limit + 1];
