...
export function boundedPositiveIntSchema(min, max) {
    return positiveIntBaseSchema.refine(value => value >= min && value <= max, `Expected an integer between ${min} and ${max}.`);
}
...
export function commaSeparatedNumericIdsSchema({ maxItems, } = {}) {
    const baseSchema = z
        .string()
        .trim()
        .transform(value => value
        ...
        .filter(Boolean))
        .refine(value => value.length > 0, 'Expected at least one id.')
        .refine(value => value.every(item => /^\d+$/.test(item)), 'Expected only numeric ids.');
    if (typeof maxItems !== 'number') {
        return baseSchema;
    }
    return baseSchema.refine(value => value.length <= maxItems, `Expected at most ${maxItems} ids.`);
}
