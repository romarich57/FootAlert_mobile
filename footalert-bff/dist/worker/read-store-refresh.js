...
import { BOOTSTRAP_DEFAULT_DISCOVERY_LIMIT, extractPlayerSeedEntriesFromCompetitionPayload, extractPlayerSeedEntriesFromTeamPayload, extractTeamSeedEntriesFromCompetitionPayload, HOTSET_COMPETITION_IDS, isCompetitionFullPayloadComplete, isMatchFullPayloadComplete, isPlayerFullPayloadComplete, isTeamFullPayloadComplete, persistWorkerMatchOverlay, READ_STORE_BOOTSTRAP_WARM_INTERVAL_MS, READ_STORE_DEFAULT_TIMEZONE, READ_STORE_REFRESH_CLAIM_LIMIT, resolveReadStoreRefreshServices, } from './read-store-refresh-support.js';
...
export function createReadStoreRefreshRuntime(input) {
    ...
    async function publishWorkerHeartbeat(source = 'read_store_refresh_cycle') {
        try {
    ...
    function assertCompleteSnapshot(entityKind, payload) {
        const isComplete = entityKind === 'competition_full'
            ? isCompetitionFullPayloadComplete(payload)
            : entityKind === 'team_full'
                ? isTeamFullPayloadComplete(payload)
                : entityKind === 'player_full'
                    ? isPlayerFullPayloadComplete(payload)
                    : isMatchFullPayloadComplete(payload);
        if (!isComplete) {
            throw new Error(`${entityKind} snapshot is incomplete`);
        }
    }
    async function getFreshEntitySnapshotPayload(inputSnapshot) {
        const snapshot = await input.readStore.getEntitySnapshot({
    ...
    async function refreshSnapshotForJob(job) {
        ...
        if (job.entityKind === 'competition_full') {
            const payload = await services.buildCompetitionFullResponse(job.entityId, parseOptionalNumber(scope.season));
    ...
    async function warmHotset() {
        ...
        for (let i = 0; i < orderedTeamEntries.length; i += 3) {
            ...
            const results = await Promise.allSettled(orderedTeamEntries.slice(i, i + 3).map(async (entry) => {
        ...
        for (let i = 0; i < orderedPlayerEntries.length; i += 5) {
            ...
            const results = await Promise.allSettled(orderedPlayerEntries.slice(i, i + 5).map(async (entry) => {
