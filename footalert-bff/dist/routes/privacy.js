...
import { consumeMobileSessionChallenge, } from '../lib/mobileSessionChallengeStore.js';
...
function isAtLeastDeviceIntegrity(integrity) {
    return integrity === 'strong' || integrity === 'device';
}
function createMemoryPrivacyAuditStore() {
    return {
        async recordErase() {
            return undefined;
        },
...
async function createPostgresPrivacyAuditStore(databaseUrl) {
...
async function createPrivacyAuditStore() {
...
export async function registerPrivacyRoutes(app) {
    ...
    app.post('/v1/mobile/privacy/erase', async (request, reply) => {
        ...
        if (challenge.platform !== payload.platform
