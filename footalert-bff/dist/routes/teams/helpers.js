export function normalizeStandingsPayload(payload) {
    if (!payload || !Array.isArray(payload.response)) {
        return { response: [] };
    }
    return payload;
}
export function buildFixtureQuery(teamId, query) {
...
function pushTeamNameCandidate(candidates, value) {
...
export function buildTeamNameCandidates(teamName) {
    ...
    for (const baseVariant of baseVariants) {
        ...
        const strippedSuffix = normalizeTeamName(baseVariant.replace(/\b(fc|cf|sc|ac|afc|fk|sk|nk|ssc|cfc)\b\.?/gi, ''));
...
export function normalizeTransferKeyText(value) {
    return value
        .trim()
        .replace(/\s+/g, ' ')
        .normalize('NFD')
