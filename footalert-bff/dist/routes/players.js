...
export function mapPlayerMatchPerformance(playerId, fixtureDto, performanceDto) {
    ...
    if (performanceDto?.players) {
        for (const teamPlayers of performanceDto.players) {
            const foundPlayer = teamPlayers.players?.find(candidate => String(candidate.player?.id) === playerId);
...
export async function registerPlayersRoutes(app) {
    app.get('/v1/players/:id', async (request) => {
        ...
        return withCache(`players:details:${request.url}`, 60_000, () => apiFootballGet(`/players?id=${encodeURIComponent(params.id)}&season=${encodeURIComponent(String(query.season))}`));
    });
    app.get('/v1/players/:id/seasons', async (request) => {
        const params = parseOrThrow(playerIdParamsSchema, request.params);
        parseOrThrow(z.object({}).strict(), request.query);
        return withCache(`players:seasons:${request.url}`, 120_000, () => apiFootballGet(`/players/seasons?player=${encodeURIComponent(params.id)}`));
    });
    app.get('/v1/players/:id/trophies', async (request) => {
        const params = parseOrThrow(playerIdParamsSchema, request.params);
        parseOrThrow(z.object({}).strict(), request.query);
        return withCache(`players:trophies:${request.url}`, 120_000, () => apiFootballGet(`/trophies?player=${encodeURIComponent(params.id)}`));
    });
    app.get('/v1/players/:id/career', async (request) => {
        const params = parseOrThrow(playerIdParamsSchema, request.params);
        parseOrThrow(z.object({}).strict(), request.query);
        return withCache(`players:career:${request.url}`, 120_000, async () => {
            const seasonsPayload = await apiFootballGet(`/players/seasons?player=${encodeURIComponent(params.id)}`);
            ...
            const detailsPayloads = await Promise.all(seasons.map(async (season) => {
                try {
                    const payload = await withCache(`players:details:career:${params.id}:${season}`, 60_000, () => apiFootballGet(`/players?id=${encodeURIComponent(params.id)}&season=${encodeURIComponent(String(season))}`));
    ...
    app.get('/v1/players/team/:teamId/fixtures', async (request) => {
        ...
        return withCache(`players:teamfixtures:${request.url}`, 45_000, () => apiFootballGet(`/fixtures?${searchParams.toString()}`));
    });
    app.get('/v1/players/fixtures/:fixtureId/team/:teamId/stats', async (request) => {
        ...
        parseOrThrow(z.object({}).strict(), request.query);
        return withCache(`players:fixturestats:${request.url}`, 45_000, () => apiFootballGet(`/fixtures/players?fixture=${encodeURIComponent(params.fixtureId)}&team=${encodeURIComponent(params.teamId)}`));
    });
    app.get('/v1/players/:id/matches', async (request) => {
        ...
        return withCache(`players:matches:${request.url}`, 45_000, async () => {
            const fixturesPayload = await apiFootballGet(`/fixtures?team=${encodeURIComponent(query.teamId)}&season=${encodeURIComponent(String(query.season))}&last=${encodeURIComponent(String(lastCount))}`);
            ...
            const performances = await Promise.all(fixtures
                ...
                try {
                    const fixtureStatsPayload = await withCache(`players:fixturestats:aggregate:${fixtureId}:${query.teamId}`, 45_000, () => apiFootballGet(`/fixtures/players?fixture=${encodeURIComponent(fixtureId)}&team=${encodeURIComponent(query.teamId)}`));
