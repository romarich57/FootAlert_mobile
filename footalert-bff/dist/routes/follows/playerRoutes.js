...
import { cardsPlayerIdsQuerySchema, playerSeasonParamsSchema, } from './schemas.js';
export function registerFollowsPlayerRoutes(app) {
    ...
    app.get('/v1/follows/players/cards', async (request) => {
        ...
        return withCache(buildCanonicalCacheKey('follows:player-cards', {
            ids: query.ids,
            season: query.season,
        }), 60_000, async () => {
            const cards = await mapWithConcurrency(query.ids, FOLLOW_CARDS_CONCURRENCY, async (playerId) => {
                const payload = await withCache(buildCanonicalCacheKey('follows:player-season', {
                    playerId,
                    season: query.season,
                }), 60_000, () => apiFootballGet(`/players?id=${encodeURIComponent(playerId)}&season=${encodeURIComponent(String(query.season))}`));
