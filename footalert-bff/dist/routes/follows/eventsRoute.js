...
import { FOLLOW_DISCOVERY_CACHE_TTL_MS, FOLLOW_DISCOVERY_DEFAULT_LIMIT, FOLLOW_DISCOVERY_MAX_LEAGUE_COUNT, FOLLOW_DISCOVERY_PLAYERS_BLOCKING_LEAGUE_COUNT, FOLLOW_DISCOVERY_SYNC_BUDGET_MS, FOLLOW_DISCOVERY_TEAMS_BLOCKING_LEAGUE_COUNT, } from './constants.js';
import { discoveryQuerySchema, followEventSchema, } from './schemas.js';
import { toMetadataFromSnapshot } from './discovery/mappers.js';
import { buildDiscoveryResponse, getCurrentSeasonYear, hydrateDiscoveryMetadataIfNeeded, loadLegacyPlayerDiscovery, loadLegacyTeamDiscovery, mapDynamicPlayerDiscoveryItems, mapDynamicTeamDiscoveryItems, scheduleDiscoveryCacheRefresh, waitForResultWithinBudget, } from './discovery/service.js';
function rejectUnauthorizedFollowEventRequest(request, reply) {
...
async function recordFollowEvent(store, payload, subject) {
    const occurredAt = payload.occurredAt ? new Date(payload.occurredAt) : undefined;
    const normalizedEntityId = payload.entityKind === 'player'
...
export function registerFollowsEventsRoute(app, getDiscoveryStore) {
    app.post('/v1/follows/events', async (request, reply) => {
        ...
        const { eventResult, normalizedEntityId } = await recordFollowEvent(store, payload, authContext.subject);
        if (payload.entitySnapshot) {
            await store.upsertMetadata(payload.entityKind, normalizedEntityId, toMetadataFromSnapshot(payload.entityKind, normalizedEntityId, payload.entitySnapshot));
        }
        else {
...
export function registerFollowsDiscoveryRoutes(app, getDiscoveryStore) {
    app.get('/v1/follows/discovery/teams', async (request) => {
        ...
        return withCacheStaleWhileRevalidate(cacheKey, FOLLOW_DISCOVERY_CACHE_TTL_MS, async () => {
            const store = await getDiscoveryStore();
            ...
            const blockingLegacyPromise = loadLegacyTeamDiscovery(limit, season, request.log, FOLLOW_DISCOVERY_TEAMS_BLOCKING_LEAGUE_COUNT);
            const fullLegacyPromise = loadLegacyTeamDiscovery(limit, season, request.log, FOLLOW_DISCOVERY_MAX_LEAGUE_COUNT);
            const blockingLegacyItems = await waitForResultWithinBudget(blockingLegacyPromise, FOLLOW_DISCOVERY_SYNC_BUDGET_MS);
            if (blockingLegacyItems === null) {
                ...
                scheduleDiscoveryCacheRefresh({
                    cacheKey,
                    logger: request.log,
                    refreshPromise: fullLegacyPromise.then(legacyItems => buildDiscoveryResponse({
                        dynamicItems,
                        legacyItems,
                        staticItems,
                        getId: item => item.teamId,
    ...
    app.get('/v1/follows/discovery/players', async (request) => {
        ...
        return withCacheStaleWhileRevalidate(cacheKey, FOLLOW_DISCOVERY_CACHE_TTL_MS, async () => {
            ...
            const blockingLegacyPromise = loadLegacyPlayerDiscovery(limit, season, request.log, FOLLOW_DISCOVERY_PLAYERS_BLOCKING_LEAGUE_COUNT);
            const fullLegacyPromise = loadLegacyPlayerDiscovery(limit, season, request.log, FOLLOW_DISCOVERY_MAX_LEAGUE_COUNT);
            const blockingLegacyItems = await waitForResultWithinBudget(blockingLegacyPromise, FOLLOW_DISCOVERY_SYNC_BUDGET_MS);
            if (blockingLegacyItems === null) {
                const immediateResponse = buildDiscoveryResponse({
                    dynamicItems,
