...
export function mapPlayerDetailsToProfile(dto, season) {
    if (!dto.player) {
        return null;
    }
...
export function mapPlayerDetailsToCharacteristics(dto, season) {
    const statistic = resolvePrimaryStatistic(dto.statistics, season);
    if (!statistic) {
        return {
            touches: null,
    ...
    const touches = sumOrNull(normalizeNumber(statistic.passes?.total), normalizeNumber(statistic.dribbles?.attempts));
    const dribbles = normalizeNumber(statistic.dribbles?.success);
    const chances = normalizeNumber(statistic.passes?.key);
    const defense = sumOrNull(normalizeNumber(statistic.tackles?.total), normalizeNumber(statistic.tackles?.interceptions));
    const duels = normalizeNumber(statistic.duels?.won);
    const attack = sumOrNull(normalizeNumber(statistic.goals?.total), normalizeNumber(statistic.shots?.on));
    return { touches, dribbles, chances, defense, duels, attack };
}
function toRatingWeight(value) {
    if (!value) {
...
function compareByMostPlayed(first, second) {
...
export function selectProfileCompetitionStats(seasonStatsDataset) {
    if (!seasonStatsDataset || seasonStatsDataset.byCompetition.length === 0) {
        return null;
    }
    const mappedStats = seasonStatsDataset.byCompetition.map(competition => ({
    ...
    const seasonScoped = latestSeason !== null
