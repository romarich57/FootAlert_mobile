...
import { competitionMatchesQuerySchema, DEFAULT_PAGINATION_LIMIT, } from './schemas.js';
import { buildOrderedFixtureRounds, flattenOrderedFixtureRounds, } from './fixtureOrdering.js';
import { buildCompetitionMatchesPageInfo, decodeCompetitionMatchesCursor, resolveSmartAnchorStartPosition, } from './pagination.js';
export function registerCompetitionMatchesRoute(app) {
    app.get('/v1/competitions/:id/matches', {
        config: {
            rateLimit: {
                max: 30,
                timeWindow: '1 minute',
        ...
        const payload = await withCache(upstreamCacheKey, 90_000, () => apiFootballGet(`/fixtures?league=${encodeURIComponent(params.id)}&season=${encodeURIComponent(String(query.season))}`));
        ...
        const orderedRounds = buildOrderedFixtureRounds((Array.isArray(payload.response) ? payload.response : []));
