...
function parseRoundOfValue(normalizedRoundName) {
...
function parseNumberedRound(normalizedRoundName) {
...
export function classifyRound(roundName) {
...
export function detectCompetitionKind(groupNames, knockoutRoundNames) {
...
export function getRoundOrder(roundName) {
    const normalized = normalizeRoundName(roundName);
    if (!normalized)
    ...
    if (normalized.includes('qualifying'))
        return 7_500;
    if (/\b(play-?off|barrage)\b/.test(normalized))
        return 9_900;
    return UNKNOWN_ROUND_ORDER;
}
