...
const teamPlayersQuerySchema = z
    ...
    .strict();
function buildFixtureQuery(teamId, query) {
...
export async function registerTeamsRoutes(app) {
    app.get('/v1/teams/standings', {
        ...
        return withCache(`team:standings:${request.url}`, 60_000, () => apiFootballGet(`/standings?league=${encodeURIComponent(query.leagueId)}&season=${encodeURIComponent(String(query.season))}`));
    });
    app.get('/v1/teams/:id', async (request) => {
        ...
        return withCache(`team:details:${request.url}`, 120_000, () => apiFootballGet(`/teams?id=${encodeURIComponent(params.id)}`));
    });
    app.get('/v1/teams/:id/leagues', async (request) => {
        const params = parseOrThrow(teamIdParamsSchema, request.params);
        parseOrThrow(z.object({}).strict(), request.query);
        return withCache(`team:leagues:${request.url}`, 120_000, () => apiFootballGet(`/leagues?team=${encodeURIComponent(params.id)}`));
    });
    app.get('/v1/teams/:id/fixtures', async (request) => {
        const params = parseOrThrow(teamIdParamsSchema, request.params);
        const query = parseOrThrow(teamFixturesQuerySchema, request.query);
        return withCache(`team:fixtures:${request.url}`, 45_000, () => apiFootballGet(`/fixtures?${buildFixtureQuery(params.id, query)}`));
    });
    ...
    app.get('/v1/teams/:id/squad', async (request) => {
        const params = parseOrThrow(teamIdParamsSchema, request.params);
        parseOrThrow(z.object({}).strict(), request.query);
        return withCache(`team:squad:${request.url}`, 120_000, () => apiFootballGet(`/players/squads?team=${encodeURIComponent(params.id)}`));
    });
    app.get('/v1/teams/:id/transfers', async (request) => {
        const params = parseOrThrow(teamIdParamsSchema, request.params);
        parseOrThrow(z.object({}).strict(), request.query);
        return withCache(`team:transfers:${request.url}`, 120_000, () => apiFootballGet(`/transfers?team=${encodeURIComponent(params.id)}`));
    });
    app.get('/v1/teams/:id/trophies', async (request) => {
        const params = parseOrThrow(teamIdParamsSchema, request.params);
        parseOrThrow(z.object({}).strict(), request.query);
        return withCache(`team:trophies:${request.url}`, 120_000, () => apiFootballGet(`/trophies?team=${encodeURIComponent(params.id)}`));
    });
}
