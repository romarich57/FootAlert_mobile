...
import {
  ...
  defaultCacheBackendForEnv,
  defaultNotificationsPersistenceBackendForEnv,
} from './defaults.js';
import {
  buildCorsAllowedOrigins,
  normalizeUrl,
  readAppEnv,
  readAttestationEnforcementMode,
  readBoolean,
  readCacheBackend,
  readHostList,
  readNodeRole,
  readNonNegativeInt,
  readNotificationsPersistenceBackend,
  readOptionalOrigin,
  readOptionalValue,
  readPositiveInt,
  readRedisCachePrefix,
  readRequiredTrimmedValue,
} from './readers.js';
...
export function buildEnv(source: NodeJS.ProcessEnv = process.env): BffEnv {
  ...
  const defaultNotificationsPersistenceBackend =
    defaultNotificationsPersistenceBackendForEnv(appEnv);
  ...
  const env: BffEnv = {
    ...
    apiFootballKey: readRequiredTrimmedValue(
      source.API_FOOTBALL_KEY,
      'Missing API_FOOTBALL_KEY in BFF environment.',
    ),
    ...
    trustProxyHops: Math.max(
      readPositiveInt(source.TRUST_PROXY_HOPS, DEFAULT_TRUST_PROXY_HOPS),
      0,
    ),
    webAppOrigin,
    ...
    mobilePlayIntegrityServiceAccountEmail: readOptionalValue(
      source.MOBILE_PLAY_INTEGRITY_SERVICE_ACCOUNT_EMAIL,
    ),
    mobilePlayIntegrityServiceAccountPrivateKey: readOptionalValue(
      source.MOBILE_PLAY_INTEGRITY_SERVICE_ACCOUNT_PRIVATE_KEY,
    ),
