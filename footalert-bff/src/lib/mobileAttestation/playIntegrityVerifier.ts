...
function signServiceAccountAssertion(assertion: string, privateKey: string): string | null {
  try {
...
async function getPlayIntegrityAccessToken(): Promise<string | null> {
  ...
  const assertionClaims = encodeBase64Url(
    JSON.stringify({
  ...
  const parsed = (await tokenResponse.json()) as GoogleTokenResponse;
...
async function decodeIntegrityToken(
  ...
  accessToken: string,
): Promise<DecodeIntegrityResponse | null> {
  if (!env.mobilePlayIntegrityPackageName) {
    return null;
  }

  const response = await fetch(
    `https://playintegrity.googleapis.com/v1/${encodeURIComponent(env.mobilePlayIntegrityPackageName)}:decodeIntegrityToken`,
    {
      method: 'POST',
      headers: {
...
function parseNonce(nonce: string): ParsedNonce | null {
  ...
  try {
    const parsed = JSON.parse(decoded) as Partial<ParsedNonce>;
    if (
...
function resolveIntegrityFromVerdict(verdicts: string[] | undefined): 'strong' | 'device' | 'basic' | 'unknown' {
...
function isNonceCompatible(input: VerifyMobileAttestationInput, nonce: string | undefined): boolean {
...
export async function verifyPlayIntegrityToken(
  ...
  if (
    (requestPackageName && requestPackageName !== env.mobilePlayIntegrityPackageName)
