...
import {
  createReadStoreRefreshRuntime,
} from './worker/read-store-refresh.js';
import { createNotificationQueues, importBullMq } from './worker/queue-runtime.js';
import {
  createWorkerServiceLogger,
  type JobLike,
  logWorker,
...
async function startWorker(): Promise<void> {
  ...
  const dispatchWorker = new WorkerConstructor(
    NOTIFICATIONS_DISPATCH_QUEUE,
    notificationJobRuntime.handleDispatchJob,
    { connection, concurrency: 5 },
  );
  const deferredPromotionWorker = new WorkerConstructor(
    NOTIFICATIONS_DEFERRED_PROMOTION_QUEUE,
    notificationJobRuntime.handleDeferredPromotionJob,
    { connection, concurrency: 2 },
  );
  const sendWorker = new WorkerConstructor(
    NOTIFICATIONS_SEND_QUEUE,
    notificationJobRuntime.handleSendJob,
    { connection, concurrency: 10 },
  );

  dispatchWorker.on('failed', (job, error) =>
    notificationJobRuntime.handleDispatchFailed(
      job as JobLike<DispatchJobPayload> | undefined,
      error,
    ));
  deferredPromotionWorker.on('failed', (job, error) =>
    notificationJobRuntime.handleDeferredPromotionFailed(
      job as JobLike<DeferredPromotionJobPayload> | undefined,
      error,
    ));
  sendWorker.on('failed', (job, error) =>
    notificationJobRuntime.handleSendFailed(job as JobLike<SendJobPayload> | undefined, error));
