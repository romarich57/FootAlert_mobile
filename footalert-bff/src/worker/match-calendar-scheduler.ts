...
type FixtureScheduleEntry = {
  fixtureId: string;
  kickoffMs: number | null;
  lifecycleState: 'pre_match' | 'imminent' | 'live' | 'post_match' | 'settled';
...
function resolveFixtureLifecycle(
  statusShort: string,
  kickoffMs: number | null,
  nowMs: number,
): FixtureScheduleEntry['lifecycleState'] {
...
function parseFixtureScheduleEntries(
  fixtures: unknown[],
  nowMs: number,
  ...
  for (const item of fixtures) {
    ...
    const statusObj = fixtureObj?.status && typeof fixtureObj.status === 'object'
      ? (fixtureObj.status as Record<string, unknown>)
      : null;
    ...
    const homeTeam = teamsObj?.home && typeof teamsObj.home === 'object'
      ? (teamsObj.home as Record<string, unknown>)
      : null;
    const awayTeam = teamsObj?.away && typeof teamsObj.away === 'object'
      ? (teamsObj.away as Record<string, unknown>)
      : null;
...
async function fetchTodayFixtures(
  ...
  const payload = await withCache<MatchesTodayResponse>(
    buildCanonicalCacheKey('calendar:matches-today', { date, timezone }),
    MATCHES_TODAY_CACHE_TTL_MS,
    () =>
...
async function enqueueMatchRefresh(
  readStore: ReadStore,
  entry: FixtureScheduleEntry,
  timezone: string,
  priority: number,
  notBefore: Date,
): Promise<void> {
...
async function enqueueTeamWarm(
  readStore: ReadStore,
  teamId: string,
  leagueId: string | null,
  season: number,
...
export async function runCalendarScheduleCycle(input: {
  readStore: ReadStore;
  timezone: string;
}): Promise<{ scheduled: number; live: number; imminent: number }> {
