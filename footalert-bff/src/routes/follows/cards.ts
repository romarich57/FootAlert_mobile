import type { PlayerSeasonResponse, TeamDetailsResponse, TeamNextFixtureResponse } from './schemas.js';

export function mapTeamCard(
  teamId: string,
  ...
  return {
    ...
    teamLogo: team?.logo ?? '',
    nextMatch:
      nextFixture?.fixture?.id && nextFixture?.fixture?.date
        ? {
...
export function mapPlayerCard(playerId: string, payload: PlayerSeasonResponse) {
