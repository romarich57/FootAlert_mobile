...
type MatchFullPayload = Awaited<ReturnType<typeof buildMatchFullResponse>>;

type MatchLiveOverlayPayload = {
  fixture: MatchFullPayload['fixture'];
  lifecycleState: MatchFullPayload['lifecycleState'];
  context: MatchFullPayload['context'];
  events: MatchFullPayload['events'];
  statistics: MatchFullPayload['statistics'];
  lineups: MatchFullPayload['lineups'];
  absences: MatchFullPayload['absences'];
  playersStats: MatchFullPayload['playersStats'];
};

const LIVE_OVERLAY_STALE_MS = 15_000;
const LIVE_OVERLAY_EXPIRES_MS = 120_000;

function toMatchLiveOverlay(payload: MatchFullPayload): MatchLiveOverlayPayload {
  return {
...
function applyMatchLiveOverlay(
  payload: MatchFullPayload,
  overlay: MatchLiveOverlayPayload,
): MatchFullPayload {
...
export function registerMatchFullRoute(app: FastifyInstance): void {
  app.get(
    ...
    async (request, reply) => {
      ...
      const persistLiveOverlay = async (payload: MatchFullPayload): Promise<void> => {
      ...
      const overlaySnapshot = await readStore.getMatchLiveOverlay<MatchLiveOverlayPayload>({
        matchId: params.id,
      });
