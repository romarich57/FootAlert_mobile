...
import type {
  ...
  CompetitionStandingSplit,
  CompetitionStandingsPayload,
  StandingSplitRaw,
} from './contracts.js';
...
export function normalizeStandingRows(
  payload: CompetitionStandingsPayload,
): CompetitionStandingRow[] {
...
export function isGroupedCompetition(payload: CompetitionStandingsPayload): boolean {
  const groups = payload.response?.[0]?.league?.standings ?? [];
  return groups.length > 1;
}

export function mapStandingRowsToComputedTeamStats(
