...
import {
  ADVANCED_METRICS,
  ADVANCED_SORT_ORDERS,
  HOME_AWAY_METRICS,
  HOME_AWAY_SORT_ORDERS,
  SUMMARY_METRICS,
  SUMMARY_SORT_ORDERS,
} from './contracts.js';
...
export function buildSummarySection<T extends Record<SummaryMetricKey, number | null> & {
  teamId: number;
  teamName: string;
  teamLogo: string;
}>(rows: T[]): CompetitionTeamStatsSection<SummaryMetricKey> {
  return {
...
export function buildHomeAwaySection<T extends Record<HomeAwayMetricKey, number | null> & {
  teamId: number;
  teamName: string;
  teamLogo: string;
}>(rows: T[]): CompetitionTeamStatsSection<HomeAwayMetricKey> {
  return {
...
export function buildEmptyAdvancedLeaderboards(): CompetitionTeamAdvancedSection['leaderboards'] {
  return {
