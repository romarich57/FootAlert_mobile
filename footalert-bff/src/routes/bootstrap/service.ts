...
} from '../follows/discovery/service.js';
import type {
  FollowDiscoveryPlayerItem,
  FollowDiscoveryRouteResponse,
  FollowDiscoveryTeamItem,
  PlayerSeasonResponse,
  TeamDetailsResponse,
  TeamNextFixtureResponse,
} from '../follows/schemas.js';

type MatchesTodayResponse = {
  response?: unknown[];
};

type WarmEntityKind = 'team' | 'player' | 'competition' | 'match';

export type WarmEntityRef = {
  kind: WarmEntityKind;
  id: string;
};
...
export type BootstrapPayload = {
  ...
  followedTeamCards: ReturnType<typeof mapTeamCard>[];
  followedPlayerCards: ReturnType<typeof mapPlayerCard>[];
  discovery: {
    teams: FollowDiscoveryRouteResponse<FollowDiscoveryTeamItem>;
    players: FollowDiscoveryRouteResponse<FollowDiscoveryPlayerItem>;
  };
  warmEntityRefs: WarmEntityRef[];
};
...
export function buildBootstrapScopeKey(input: {
  ...
  const normalizeIds = (value: readonly string[] | undefined): string[] =>
    Array.from(
      new Set(
        (value ?? [])
          .map(item => item.trim())
...
export function parseBootstrapScopeKey(scopeKey: string): {
  date: string;
  timezone: string;
  season: number;
  discoveryLimit: number;
...
function readTimezoneDate(now: Date, timezone: string): string {
...
function readFixtureWarmRefs(
  fixtures: unknown[],
): WarmEntityRef[] {
  const refs: WarmEntityRef[] = [];
  ...
  for (const fixtureEntry of fixtures) {
    ...
    const fixtureRecord = fixtureEntry as Record<string, unknown>;
    const fixture =
      fixtureRecord.fixture && typeof fixtureRecord.fixture === 'object'
        ? (fixtureRecord.fixture as Record<string, unknown>)
        : null;
    const league =
      fixtureRecord.league && typeof fixtureRecord.league === 'object'
        ? (fixtureRecord.league as Record<string, unknown>)
        : null;
    const teams =
      fixtureRecord.teams && typeof fixtureRecord.teams === 'object'
        ? (fixtureRecord.teams as Record<string, unknown>)
        : null;
    const homeTeam =
      teams?.home && typeof teams.home === 'object'
        ? (teams.home as Record<string, unknown>)
        : null;
    const awayTeam =
      teams?.away && typeof teams.away === 'object'
...
function dedupeWarmEntityRefs(
  refs: WarmEntityRef[],
  limit = 250,
): WarmEntityRef[] {
  const deduped: WarmEntityRef[] = [];
...
async function fetchMatchesToday(date: string, timezone: string): Promise<unknown[]> {
  const payload = await withCache<MatchesTodayResponse>(
    buildCanonicalCacheKey('bootstrap:matches-today', { date, timezone }),
    30_000,
    () =>
      apiFootballGet<MatchesTodayResponse>(
...
async function fetchFollowedTeamCards(
  followedTeamIds: string[],
  timezone: string,
): Promise<ReturnType<typeof mapTeamCard>[]> {
  if (followedTeamIds.length === 0) {
    return [];
  }
  ...
  return mapWithConcurrency(followedTeamIds, FOLLOW_CARDS_CONCURRENCY, async teamId => {
    const [detailsPayload, nextFixturePayload] = await Promise.all([
      withCache<TeamDetailsResponse>(
        buildCanonicalCacheKey('bootstrap:team-details', { teamId }),
        120_000,
        () => apiFootballGet(`/teams?id=${encodeURIComponent(teamId)}`),
      ),
      withCache<TeamNextFixtureResponse>(
        buildCanonicalCacheKey('bootstrap:team-next-fixture', {
          teamId,
          timezone,
        }),
        45_000,
...
async function fetchFollowedPlayerCards(
  followedPlayerIds: string[],
  season: number,
): Promise<ReturnType<typeof mapPlayerCard>[]> {
  if (followedPlayerIds.length === 0) {
    return [];
  }
  ...
  return mapWithConcurrency(followedPlayerIds, FOLLOW_CARDS_CONCURRENCY, async playerId => {
    const payload = await withCache<PlayerSeasonResponse>(
      buildCanonicalCacheKey('bootstrap:player-season', { playerId, season }),
      60_000,
      () =>
        apiFootballGet(
...
async function loadTeamDiscovery(
  limit: number,
  season: number,
  store: FollowsDiscoveryStore | null,
  logger?: FastifyBaseLogger,
): Promise<FollowDiscoveryRouteResponse<FollowDiscoveryTeamItem>> {
  const dynamicItems =
    store === null
      ? []
      : mapDynamicTeamDiscoveryItems(await store.getDiscovery('team', limit));
  const legacyItems = await loadLegacyTeamDiscovery(
    limit,
    season,
    logger,
    FOLLOW_DISCOVERY_TEAMS_BLOCKING_LEAGUE_COUNT,
...
async function loadPlayerDiscovery(
  ...
  store: FollowsDiscoveryStore | null,
  logger?: FastifyBaseLogger,
): Promise<FollowDiscoveryRouteResponse<FollowDiscoveryPlayerItem>> {
  const dynamicItems =
    store === null
      ? []
      : mapDynamicPlayerDiscoveryItems(await store.getDiscovery('player', limit));
  const legacyItems = await loadLegacyPlayerDiscovery(
    limit,
    season,
    logger,
    FOLLOW_DISCOVERY_PLAYERS_BLOCKING_LEAGUE_COUNT,
...
export async function buildBootstrapPayload(input: {
  ...
  followedPlayerIds: string[];
  discoveryLimit?: number;
  discoveryStore?: FollowsDiscoveryStore | null;
  logger?: FastifyBaseLogger;
  now?: Date;
  ...
  const warmEntityRefs = dedupeWarmEntityRefs([
    ...
    ...input.followedTeamIds.map(teamId => ({ kind: 'team' as const, id: teamId })),
    ...input.followedPlayerIds.map(playerId => ({ kind: 'player' as const, id: playerId })),
